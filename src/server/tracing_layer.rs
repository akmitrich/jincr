...
static REQ_NUM: atomic::AtomicU64 = atomic::AtomicU64::new(1);
...
impl tracing_actix_web::RootSpanBuilder for ServerRootSpanBuilder {
    #[tracing::instrument(skip_all, name="chat", fields(req_num=(REQ_NUM.fetch_add(1, atomic::Ordering::Relaxed))))]
    ...
    fn on_request_end<B: body::MessageBody>(
        ...
        let response = match outcome {
            Ok(outcome) => format!("{:?}", outcome.response())
                .replace("\n", "")
                .replace("\r", ""),
